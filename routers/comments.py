...
async def fetch_and_analyze_comments(req: CommentsRequest):
    ...
    if comments:
        analysis = groq.analyze_real_comments(comments, req.business_profile)
        ...
        print(f"[Comments] {len(comments)} comments found but no useful text (emoji-only?) — falling back to captions")

    # Fallback: estimate from video captions using Groq
    captions = req.captions or []
    if not captions:
        raise HTTPException(
            status_code=404,
            detail="تعذّر جلب التعليقات — الفيديوهات محمية على TikTok"
        )

    analysis = groq.analyze_audience_from_captions(captions, req.business_profile)
    if not analysis:
